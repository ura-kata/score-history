...
namespace ScoreHistoryApi.Tests.WithDocker.Logics.Score.ScoreDescriptionSetterTests
{
    ...
    public class UpdateDescriptionAsyncTests
    {
        ...
        public UpdateDescriptionAsyncTests(DynamoDbDockerFixture fixture, ITestOutputHelper helper)
        {
            _fixture = fixture;
            _helper = helper;
            ...
            _services.AddScoped<ScoreDescriptionSetter>();
        ...
        public async Task Success()
        {
            ...
            var logic = provider.GetRequiredService<ScoreDescriptionSetter>();

            // 実行
            await logic.UpdateDescriptionAsync(_ownerId, scoreId, newDescription);
...

            {
                ...
                Assert.Equal(newDescription, response.Items[0]["d"].M["d"].S);
            }
        }
    }
}
