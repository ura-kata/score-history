...
const UploadDialog = (props: UploadDialogProps) => {
  ...
  const handlerUpload = useCallback(async () => {
    ...
    uploadScoreName,
    uploadScoreTitle,
    uploadScoreDescription,
    fileDataList,
    handleUploadDialogClose,
  ]);

  const uploadDialog = (
    <Dialog open={open} onClose={handleUploadDialogClose} scroll={"paper"}>
      <DialogTitle>スコアをアップロードします</DialogTitle>
      <DialogContent dividers={true}>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <TextField
              value={uploadScoreName}
              onChange={handlerUploadScoreName}
              label={"スコアの名前"}
            ></TextField>
          </Grid>
          <Grid item xs={12}>
            <TextField
              value={uploadScoreTitle}
              onChange={handlerUploadScoreTitle}
              label={"スコアのタイトル"}
            ></TextField>
          </Grid>
          <Grid item xs={12}>
            <TextField
              value={uploadScoreDescription}
              onChange={handlerUploadScoreDescription}
              label={"スコアの説明"}
            ></TextField>
          </Grid>
          <Grid item xs={12}>
      ...
      <DialogActions>
        <Button variant="outlined" color="primary" onClick={handlerUpload}>
          アップロード
        </Button>
        <Button
          variant="outlined"
