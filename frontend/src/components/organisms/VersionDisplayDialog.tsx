      ...
      <DialogContent dividers={true}>
        <Grid container spacing={3} style={{ height: "100%" }}>
          <Grid item xs={12} style={{ textAlign: "center", height: "auto" }}>
            <Select onChange={handleDisplayChange} value={displayScoreVersion}>
              {versions.map((x, i) => (
                <MenuItem value={x} key={i.toString()}>
                  {x.toString()}
                </MenuItem>
              ))}
            </Select>
          ...
          <Grid item xs={12} style={{ textAlign: "center", height: "90%" }}>
            <div
              id="score-viewer-container"
              style={{ width: "90vw", height: "65vh", display: "inline-block" }}
            >
